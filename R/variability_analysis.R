...
#'
#' @description
#' The \code{estimate_variability} function calculates comprehensive biometric genetic profiles
...
#' Coefficient of Variation (PCV), Broad-Sense Heritability (H2), Genetic Advance (GA), and Genetic Advance as % of Mean (GAM).
#'
#' @details
...
#' Where \eqn{r} represents the total absolute replication count.
#'
...
#'   \code{1} for summary parameter table. Defaults to \code{1}.
#'
#' @return A structured named \code{list} containing calculated genetic variability components:
#' \item{genotypic_variance}{Estimated genotypic variance component (\eqn{V_g}).}
#' \item{phenotypic_variance}{Total phenotypic variance component (\eqn{V_p}).}
#' \item{environmental_variance}{Environmental variance/Error Mean Square (\eqn{V_e}).}
#' \item{gcv}{Genotypic Coefficient of Variation percentage (GCV\%).}
#' \item{pcv}{Phenotypic Coefficient of Variation percentage (PCV\%).}
#' \item{heritability_percentage}{Broad-Sense Heritability percentage (\eqn{H^2 \%}).}
#' \item{genetic_advance}{Expected Genetic Advance (GA) at 5\% selection intensity (\eqn{k = 2.06}).}
#' \item{gam_percentage}{Genetic Advance as a percentage of the Grand Mean (GAM\%).}
#'
...
estimate_variability <- function(anova_results, total_replications, reporting_level = 1) {
  
  # =========================================================================
  # BLOCK 1: SYSTEM DEFENSE AND STRUCTURAL GATEKEEPER
  # =========================================================================
  ...
  # =========================================================================
  # BLOCK 2: VARIANCE EXTRACTION AND DYNAMIC TARGET MATCHING
  # =========================================================================
  ...
  # Dynamic matching for Genotype row index (handles Genotype, Genotypes, Line, Cultivar, etc.)
  target_patterns  <- "(?i)Genotype|Genotypes|Line|Lines|Cultivar|Cultivars|Variety|Varieties"
  genotype_row_idx <- grep(target_patterns, anova_matrix$Source, perl = TRUE)
  
  if (length(genotype_row_idx) == 0) {
    stop("METADATA SYNTAX CRASH: Could not discover 'Genotype' or genetic treatment row in the provided ANOVA matrix.", call. = FALSE)
  }
  
  mean_sq_genotype <- as.numeric(anova_matrix$MS[genotype_row_idx[1]])
  ...
  # =========================================================================
  # BLOCK 3: BREEDING COEFFICIENTS AND GENETIC ADVANCE ESTIMATION
  # =========================================================================
  ...
  # Construct summary return payload
  res_payload <- list(
  ...
  # =========================================================================
  # BLOCK 4: CONSOLE TABLE DISPLAY PIPELINE (CLEAN & SINGLE LINES)
  # =========================================================================
  if (reporting_level >= 1) {
    
    table_grid <- data.frame(
      "Biometric Parameter" = c(
        "Genotypic Variance (Vg)",
        "Phenotypic Variance (Vp)",
        "Environmental Variance (Ve)",
        "Genotypic Coeff of Variation (GCV%)",
        "Phenotypic Coeff of Variation (PCV%)",
        "Broad-Sense Heritability (H2 %)",
        "Genetic Advance (GA)",
        "Genetic Advance as % of Mean (GAM)"
      ),
      "Estimated Value" = c(
        sprintf("%.5f", genotypic_variance_val),
        sprintf("%.5f", phenotypic_variance_val),
        sprintf("%.5f", environmental_variance_val),
        paste0(sprintf("%.2f", gcv_val), " %"),
        paste0(sprintf("%.2f", pcv_val), " %"),
        paste0(sprintf("%.2f", h2_perc), " %"),
        sprintf("%.4f", ga),
        paste0(sprintf("%.2f", gam_val), " %")
      ),
    ...
    cat(rep("-", 70), "\n", sep = "")
    print(table_grid, row.names = FALSE, right = TRUE)
    cat(rep("-", 70), "\n\n", sep = "")
  }
  
  return(invisible(res_payload))
}